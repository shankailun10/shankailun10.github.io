...
### 两种方法：1、递归实现，2. 非递归实现(队列)
#### 1. 递归
思路: 单调栈: 应用场景，通常是一维数组，然后求任一元素左边或右边比自己大的元素
    
    # 
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:

        # 单调栈: 应用场景，通常是一维数组，然后求任一元素左边或右边比自己大的元素
...
### 两种方法：1、递归实现，2. 非递归实现(队列)
#### 1. 递归
思路: dp[i][j]: s[i,j]串是否是回文串,dp[i][j] = d[i+1][j-1] && s[i] == s[j] 当j-i >= 2时。
    
    # 
    def countSubstrings(self, s: str) -> int:

        # dp[i][j]: s[i,j]串是否是回文串,dp[i][j] = d[i+1][j-1] && s[i] == s[j]
        n = len(s)
        dp = [[False for _ in range(n)] for _ in range(n)]
        dp[0][0] = True
        res = 0
...
### 两种方法：1、递归实现，2. 非递归实现(队列)
#### 1. 递归
思路: res = max((maxCount-1)*(n+1)+maxNums, len(tasks)),maxCount:相同字符的最大个数，maxNums:具有相同字符的个数
    
    # 
    def leastInterval(self, tasks: List[str], n: int) -> int:

        # 数学题
        # res = max((maxCount-1)*(n+1)+maxNums, len(tasks))
        # maxCount:相同字符的最大个数，maxNums:具有相同字符的个数
        counts = {}
        for t in tasks:
            if t not in counts:
                counts[t] = 1
        ...
        maxCount = 0
        maxNums = 0
        for t, nums in counts.items():
            if nums > maxCount:
                maxCount = nums
        for t, nums in counts.items():
            if nums == maxCount:
                maxNums += 1
        return max((maxCount-1)*(n+1)+maxNums, len(tasks))
...
### 两种方法：1、递归实现，2. 非递归实现(队列)
#### 1. 递归
思路: 优秀解析：https://leetcode-cn.com/problems/shortest-unsorted-continuous-subarray/solution/581-zui-duan-wu-xu-lian-xu-zi-shu-zu-si-8rivt/j.
从左到右寻找将最后一个降序的，即右边界；从右到左寻找最后一个升序的，即左边界。
...
### 两种方法：1、递归实现，2. 非递归实现(队列)
#### 1. 递归
思路: 前缀和：[i,j]处的和等于sum[0,j]-sum[0,i],即[0,j]处的前缀和-[0,i]处的前缀和,从[0,len(nums)],依次判断前缀和之差是否==k,并统计次数即可
    
    # 
    def subarraySum(self, nums: List[int], k: int) -> int:
        # 前缀和：[i,j]处的和等于sum[0,j]-sum[0,i],即[0,j]处的前缀和-[0,i]处的前缀和
        # 从[0,len(nums)],依次判断前缀和之差是否==k,并统计次数即可
        dic = {0:1}
        prefixNum = 0
        res = 0
        for n in nums:
            prefixNum += n
            if prefixNum - k in dic:  # 前缀和之差==k
                res += dic[prefixNum-k]
            if prefixNum not in dic:
                dic[prefixNum] = 1
            else:
                dic[prefixNum] += 1
        return res
...
### 两种方法：1、递归实现，2. 非递归实现(队列)
#### 1. 递归
思路: 精髓：求二叉树深度，这里要看每个节点的深度，然后求最大值
...
思路: 01背包。
    ...
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        sums = sum(nums)
        tar = (sums + target) // 2
        if sums < target or (sums + target) % 2 != 0:
            return 0
...
思路: 01背包
    ...
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        for num, frep in dic.items():
            if len(heap) >= k:  # 堆满, 和堆顶比较
                if frep > heap[0][0]:
                    heapq.heapreplace(heap, (frep, num))
            else:  # 入堆
