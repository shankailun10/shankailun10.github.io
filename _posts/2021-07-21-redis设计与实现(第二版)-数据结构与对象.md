...
其中，type属性记录了对象的类型，5种类型(字符串对象(REDIS_STRING)、列表对象(REDIS_LIST)、哈希对象(REDIS_HASH)、集合对象(REDIS_SET)、有序集合对象(REDIS_ZSET)); ptr 指向的底层数据结构由encoding决定，encoding和对应的底层数据结构为：
